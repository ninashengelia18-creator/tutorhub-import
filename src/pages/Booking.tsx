...
export default function Booking() {
  ...
  const [selectedDate] = useState("March 17, 2026");
      ...
      <div className="container max-w-3xl py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="rounded-xl border bg-card p-5 card-shadow mb-6">
            <h2 className="font-semibold mb-4">{t("booking.lessonDetails")}</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="flex items-center gap-3 rounded-lg bg-muted/50 p-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t("booking.time")} (60 min)</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {paymentMethods.map((method) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 rounded-lg border p-4 text-left transition-all ${
                    selectedPayment === method.id ? "border-primary bg-primary-light" : "hover:border-muted-foreground/30"
                  }`}
                >
          ...
          <div className="rounded-xl border bg-card p-5 card-shadow mb-6">
            <h2 className="font-semibold mb-3">{t("booking.orderSummary")}</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("booking.lesson60")}</span>
                <span className="font-medium tabular-nums">₾85.00</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("booking.platformFee")}</span>
