      ...
      <div className="container py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border bg-card p-6 card-shadow mb-6">
              <div className="flex items-start gap-4">
                <div className="h-20 w-20 rounded-lg bg-primary-light flex items-center justify-center text-primary font-bold text-2xl shrink-0">{tutor.avatar}</div>
                <div className="flex-1">
                  <h1 className="text-2xl font-bold">{tutor.name}</h1>
                  <p className="text-primary font-medium">{tutor.subject} {t("profile.tutor")}</p>
              ...
              <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground">{t("profile.students")}</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold tabular-nums">{tutor.lessons.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">{t("profile.lessons")}</p>
                </div>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border bg-card p-6 card-shadow mb-6">
              <h2 className="font-semibold text-lg mb-3">{t("profile.about")}</h2>
              <p className="text-sm text-muted-foreground leading-relaxed">{tutor.bio}</p>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-xl border bg-card p-6 card-shadow">
              <h2 className="font-semibold text-lg mb-4">{t("profile.reviews")}</h2>
              <div className="space-y-4">
                {reviewsData.map((review, i) => (
                  <div key={i} className="pb-4 border-b last:border-b-0 last:pb-0">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center text-xs font-medium">{review.name.charAt(0)}</div>
          ...
          <motion.aside initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="w-full lg:w-80 shrink-0">
            <div className="sticky top-20 rounded-xl border bg-card p-5 card-shadow space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {timeSlots.map((slot) => (
                    <button key={slot} className="rounded-md border px-2 py-1.5 text-xs font-medium tabular-nums hover:border-primary hover:text-primary transition-colors">{slot}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2 pt-3 border-t">
                {[t("profile.freeTrial"), t("profile.cancelAnytime"), t("profile.aiClassroom")].map((feat) => (
